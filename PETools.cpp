...
bool __fastcall BuildImportTable(void* pPEBuffer, BufferType Type){
    ...
    _IMAGE_IMPORT_DESCRIPTOR* pImportTable_old = BufferInfo.pImportTable;
    while(BufferInfo.pImportTable->FirstThunk)
    {
        //获取当前DLL名称
        char* pDllName = Type==BufferType::ImageBuffer?
                (char*)pPEBuffer + BufferInfo.pImportTable->Name:
                (char*)pPEBuffer+(DWORD)RVA2FOA((void*)BufferInfo.pImportTable->Name,pPEBuffer);
        ...
        DWORD THUNKDATA32 = 0;
        DWORD OriginalFirstThunk_old=BufferInfo.pImportTable->OriginalFirstThunk;
        DWORD FirstThunk_old=BufferInfo.pImportTable->FirstThunk;
        while (true)
        {
            //读取由当前Dll导入的所有函数
            THUNKDATA32 = Type==BufferType::ImageBuffer?
                                             *(DWORD*)((char*)pPEBuffer+BufferInfo.pImportTable->OriginalFirstThunk):
                                             *(DWORD*)((char*)pPEBuffer+(DWORD)RVA2FOA((void*)BufferInfo.pImportTable->OriginalFirstThunk,pPEBuffer));
            if (!THUNKDATA32)
            {
                BufferInfo.pImportTable->OriginalFirstThunk=OriginalFirstThunk_old;
                BufferInfo.pImportTable->FirstThunk=FirstThunk_old;
                break;
            }
            ...
            DWORD* pFirstThunk = Type==BufferType::ImageBuffer?
                                 (DWORD*)((char*)pPEBuffer+BufferInfo.pImportTable->FirstThunk):
                                 (DWORD*)((char*)pPEBuffer+(DWORD)RVA2FOA((void*)BufferInfo.pImportTable->FirstThunk,pPEBuffer));
            //写入函数真实地址到IAT表
            *pFirstThunk = (DWORD)funcAddr;
            //printf("addr [%08x] wirte to [%08x]\n",funcAddr,pFirstThunk);
            BufferInfo.pImportTable->OriginalFirstThunk+=sizeof(DWORD);
            BufferInfo.pImportTable->FirstThunk+=sizeof(DWORD);
        }
        ++BufferInfo.pImportTable;
    }
    BufferInfo.pImportTable = pImportTable_old;
    return true;
}